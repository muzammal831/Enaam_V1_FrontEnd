...
function EditProduct() {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`http://3.138.38.248/Enaam_Backend_V1/public/api/admin/products/${id}`, {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://3.138.38.248/Enaam_Backend_V1/public/api/admin/products/${id}`, formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
            ...
            if (image) {
                ...
                await axios.post(`http://3.138.38.248/Enaam_Backend_V1/public/api/admin/products/${id}/upload-image`, formDataImage, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`,

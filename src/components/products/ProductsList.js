...
function ProductsList() {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get('http://3.138.38.248/Enaam_Backend_V1/public/api/admin/products', {
                    headers: {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this product?')) {
            try {
                await axios.delete(`http://3.138.38.248/Enaam_Backend_V1/public/api/admin/products/${id}`, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                });
